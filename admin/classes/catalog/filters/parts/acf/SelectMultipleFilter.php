<?php
	...
	class SelectMultipleFilter extends \Infocob\CRM\Products\Admin\Classes\Catalog\Filters\Parts\Acf\Filter implements Filter{
		...
		public function get() {
			...
			foreach ($this->acf_values as $acf_value) {
				if($this->_get !== false) {
					$selected = in_array($acf_value, ($this->_get ?? [])) ? "selected" : "";
				} else {
				...
				$acf_value_display = apply_filters("icp_post_meta_select_multiple_" . Tools::cleanString($this->acf_field), $acf_value_display, [
			...
			if($nb_options > 0) {
				return '
					' . $empty_value . '
					<div class="filter filter-selectmultiple acf ' . $this->acf_field . '">
						<label for="left-filters-' . $this->post_id . '-select-' . $this->acf_field . '">' . $this->title . '</label>
						<select name="infocob-crm-products[acf][' . $this->acf_field . '][]" id="icp-filters-' . $this->post_id . '-select-' . $this->acf_field . '" class="select-multiple" multiple="multiple">
							' . $options . '
						</select>
					</div>
				';
