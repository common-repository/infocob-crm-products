...
export class TopFiltersManager {
    ...
    saveEvent() {
        jQuery("#top-filters").off("update").on("update", () => {
            this.updateOrder();

            let base64Json = Utils.encodeConfig(this);
            jQuery("#top-filters > input[name='top-filters']").val(base64Json);
        });
    }

    load(configBase64) {
        jQuery("#top-filters > .content-top-filters").html("");
        jQuery("#top-filters .icp-loader").addClass("active");
    ...
    render() {
        this.toHTML().then((html) => {
            jQuery("#top-filters > .content-top-filters").html(html);

            this.applyEvents();

            jQuery("#top-filters .icp-loader").removeClass("active");
        });
    }
    ...
    applyEvents() {
        this.filters.forEach((filter, index) => {
            if(filter instanceof Filter) {
                ...
                jQuery(filter).off("del-top-filter").on("del-top-filter", (event, data) => {
        ...
        let addFilter = jQuery("#top-filters > div.actions-container > button.add-top-filter");
        jQuery(addFilter).off("click").on("click", (event) => {
            event.preventDefault();

            let filter = new Filter();
            filter.toHTML().then((filter_html) => {
                jQuery("#top-filters > .content-top-filters").append(filter_html);
                filter.applyEvents();
                this.addFilter(filter);
                ...
                jQuery("#top-filters-enable").prop("checked", true);
            });
        });
         ...
         * BEGIN Sorter events
         */
        let sortable_container = jQuery("#top-filters .content-top-filters");
    ...
    updateOrder() {
        if(this.filters !== undefined && this.filters.length > 0) {
            ...
            let filters = jQuery("#top-filters .top-filters-container");
            if(filters.length) {
