...
export class FilterButtonFilter {
    ...
    constructor() {
        this._id = FilterButtonFilter.id;
        this._title = {};
        this._langs = [];
    ...
    toHTML() {
        return new Promise((resolve, reject) => {
            ...
            this.langs.forEach((lang, index) => {
                titles += `<input class="all-witdh multilingual" data-name="title" data-lang="${ lang }" type="text" value="${ this.title[lang] ?? "" }" data-tippy-content="${ sprintf(_x("Title %s", "Admin view catalog post, meta-box 'left-filters'", 'infocob-crm-products'), lang) }" placeholder="${ sprintf(_x("Title %s", "Admin view catalog post, meta-box 'top-filters'", 'infocob-crm-products'), lang) }">`;
            });

            Promise.allSettled(promises).then(() => {
                // language=html
                resolve(`
                    <div class="filter button-filter" data-id="${ this.id }">
    ...
    getFilterElement() {
        return jQuery(`#top-filters .top-filters-container .filter.button-filter[data-id='${ this.id }']`).first();
    }
