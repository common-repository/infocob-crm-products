...
export class FilterAcf {
    ...
    toHTML() {
        return new Promise((resolve, reject) => {
            ...
            let acf_group_html = `<select data-name="acf-group" class="all-witdh" data-tippy-content="${ _x("Group", "Admin view catalog post, meta-box 'top-filters'", 'infocob-crm-products') }"><option value=""></option></select>`;
            if(post_type_value !== "") {
                promises.push(Utils.getAcfFieldGroupsFromPostType(post_type_value).then((acf_groups) => {
                    ...
                    acf_group_html = `
						<select data-name="acf-group" class="all-witdh" data-tippy-content="${ _x("Group", "Admin view catalog post, meta-box 'top-filters'", 'infocob-crm-products') }">
							${ options }
						</select>
					`;
                }));
            ...
            let acf_field_html = `<select data-name="acf-field" class="all-witdh" data-tippy-content="${ _x("Field", "Admin view catalog post, meta-box 'top-filters'", 'infocob-crm-products') }"><option value=""></option></select>`;
            if(this.acf_group !== "") {
            ...
            let defaults_html = `<select data-name="defaults" class="all-witdh" data-tippy-content="${ _x("Default", "Admin view catalog post, meta-box 'top-filters'", 'infocob-crm-products') }"></select>`;
                ...
                if (this.acf_field !== "") {
                    promises.push(Utils.getAcfFieldsValues(post_type_value, [this.acf_field]).then((response) => {
                        ...
                        if (acf_values.length) {
                            ...
                            defaults_html = `
                            <select data-name="defaults" class="all-witdh" ${multiple} data-tippy-content="${ _x("Default", "Admin view catalog post, meta-box 'top-filters'", 'infocob-crm-products') }">
                                ${options}
                            </select>
                        `;
                        }
            ...
            this.langs.forEach((lang, index) => {
                ...
                units += `<input class="all-witdh multilingual" data-name="unit" data-lang="${ lang }" type="text" value="${ this.unit[lang] ?? "" }" data-tippy-content="${ sprintf(_x("Unit %s", "Admin view catalog post, meta-box 'top-filters'", 'infocob-crm-products'), lang) }" placeholder="${ sprintf(_x("Unit %s", "Admin view catalog post, meta-box 'top-filters'", 'infocob-crm-products'), lang) }">`;
            });
                ...
                resolve(`
                    <div class="filter acf" data-id="${ this.id }">
                        <div class="display margin">
                            <select data-name="display" class="all-witdh" data-tippy-content="${ _x("Display", "Admin view catalog post, meta-box 'top-filters'", 'infocob-crm-products') }">
								<option value="select" ${ (this.display === "select") ? "selected" : "" }>${ _x("Select", "Admin view catalog post, meta-box 'top-filters'", 'infocob-crm-products') }</option>
								<option value="select-multiple" ${ (this.display === "select-multiple") ? "selected" : "" }>${ _x("Select multiple", "Admin view catalog post, meta-box 'top-filters'", 'infocob-crm-products') }</option>
								<option value="checkbox" ${ (this.display === "checkbox") ? "selected" : "" }>${ _x("Checkbox", "Admin view catalog post, meta-box 'top-filters'", 'infocob-crm-products') }</option>
								<option value="radio" ${ (this.display === "radio") ? "selected" : "" }>${ _x("Radio", "Admin view catalog post, meta-box 'top-filters'", 'infocob-crm-products') }</option>
								<option value="range" ${ (this.display === "range") ? "selected" : "" }>${ _x("Range", "Admin view catalog post, meta-box 'top-filters'", 'infocob-crm-products') }</option>
                            </select>
                        </div>
						...
						<div class="step margin ${(this.display !== "range") ? "invisible" : ""}">
							<input type="number" data-name="step" min="0" value="${this.step}" class="all-witdh" data-tippy-content="${ _x("Step", "Admin view catalog post, meta-box 'top-filters'", 'infocob-crm-products') }">
						</div>
    ...
    applyEvents() {
        ...
        let display = jQuery(top_filter_element).find("select[data-name='display']");
        ...
        jQuery(display).off("change").on("change", (event) => {
            ...
            if(selected_value !== previous_display) {
                ...
                let defaults = jQuery(top_filter_element).find("select[data-name='defaults']");
                ...
                if(this.display !== "range" && this.acf_field !== "") {
                    Utils.getAcfFieldsValues(post_type, [this.acf_field]).then((response) => {
                        ...
                        jQuery(top_filter_element).find("select[data-name='defaults'][multiple=multiple]").multipleSelect("refresh");
                    }).catch(() => {
                        jQuery(defaults).parents("div.margin").first().html(defaults_html);
                    }).finally(() => {
                        this.applyEvents();
        ...
        let acf_group = jQuery(top_filter_element).find("select[data-name='acf-group']");
        ...
        jQuery(acf_group).off("change").on("change", (event) => {
            ...
            if(current_value !== previous_acf_group) {
                ...
                let acf_field = jQuery(top_filter_element).find("select[data-name='acf-field']");
        ...
        let unit = jQuery(top_filter_element).find("input[data-name='unit']");
        ...
        let step = jQuery(top_filter_element).find("input[data-name='step']");
    ...
    getFilterElement() {
        return jQuery(`#top-filters .top-filters-container .filter.acf[data-id='${ this.id }']`).first();
    }
