...
export class FilterOrderBy {
                ...
                resolve(`
                    <div class="filter order-by" data-id="${ this.id }">	
						...
						<div class="actions-container add">
							<button type="button" class="add-order-by">${ _x("Add order by", "Admin view catalog post, meta-box 'right-filters'", 'infocob-crm-products') }</button>
						</div>
					</div>
                `);
            });
    ...
    applyEvents() {
        ...
        let addOrderBy = jQuery(right_filter_element).find(".actions-container.add > button.add-order-by");
        jQuery(addOrderBy).off("click").on("click", (event) => {
            event.preventDefault();

            let order_by = new OrderBy();
            order_by.toHTML().then((order_by_html) => {
                jQuery(right_filter_element).find(".content-products-order-by").append(order_by_html);
                order_by.applyEvents();
                this.addOrderBy(order_by);
         ...
         * BEGIN Sorter events
         */
        let sortable_container = jQuery(right_filter_element).find(".content-products-order-by");
    ...
    getFilterElement() {
        return jQuery(`#right-filters .right-filters-container .filter.order-by[data-id='${ this.id }']`).first();
    }
    ...
    updateOrder() {
        if(this.orders_by !== undefined && this.orders_by.length > 0) {
            ...
            let orders_by = jQuery(right_filter_element).find(".content-products-order-by .order-by-container");
            if(orders_by.length) {
