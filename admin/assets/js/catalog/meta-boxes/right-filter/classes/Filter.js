...
import {FilterButtonReset} from "../../left-filters/classes/FilterButtonReset.js";
...
export class Filter {
    ...
    constructor() {
        ...
        this._acf_enabled = Boolean(jQuery("#right-filters").data("acf"));
    ...
    toHTML() {
        return new Promise((resolve, reject) => {
            ...
            Promise.allSettled(promises).then(() => {
                resolve(`
                    <div class="right-filters-container" data-id="${ this.id }">
                        ...
                        <div class="type margin">
                            <select data-name="type" class="all-witdh" data-tippy-content="${ _x("Type", "Admin view catalog post, meta-box 'right-filters'", 'infocob-crm-products') }">
                                <option value="post_meta" ${(this.type === "post_meta") ? `selected` : ``}>${ _x("Post meta", "Admin view catalog post, meta-box 'right-filters'", 'infocob-crm-products') }</option>
                                <option value="taxonomy" ${(this.type === "taxonomy") ? `selected` : ``}>${ _x("Taxonomy", "Admin view catalog post, meta-box 'right-filters'", 'infocob-crm-products') }</option>
                                ${ this.acf_enabled ? `<option value="acf" ${(this.type === "acf") ? `selected` : ``}>${ _x("ACF", "Admin view catalog post, meta-box 'right-filters'", 'infocob-crm-products') }</option>` : ``}
                                <option value="order_by" ${(this.type === "order_by") ? `selected` : ``}>${ _x("Order by", "Admin view catalog post, meta-box 'right-filters'", 'infocob-crm-products') }</option>
                                <option value="button_filter" ${(this.type === "button_filter") ? `selected` : ``}>${ _x("Button filter", "Admin view catalog post, meta-box 'right-filters'", 'infocob-crm-products') }</option>
                                <option value="button_reset" ${(this.type === "button_reset") ? `selected` : ``}>${ _x("Button reset", "Admin view catalog post, meta-box 'right-filters'", 'infocob-crm-products') }</option>
                            </select>
                        </div>
                         ${ filter_html }
                        <div class="actions-container">
                            <div class="right-filters">
                                <button type="button" class="del-right-filter">${_x("Delete", "Admin view catalog post, meta-box 'right-filters'", 'infocob-crm-products')}</button>
                            </div>
                        </div>
                    </div>
                `);
    ...
    updateFilter() {
        let right_filter_element = this.getRightFilterElement();

        let filter_html = "";
        if (this.filter instanceof FilterPostMeta || this.filter instanceof FilterTaxonomy || this.filter instanceof FilterAcf || this.filter instanceof FilterOrderBy || this.filter instanceof FilterButtonFilter || this.filter instanceof FilterButtonReset) {
            this.filter.toHTML().then((html) => {
                filter_html = html;
                jQuery(right_filter_element).find(".filter").remove();
                jQuery(filter_html).insertBefore(jQuery(right_filter_element).find(".actions-container"))
    ...
    applyEvents() {
        ...
        let type = jQuery(right_filter_element).find("select[data-name='type']");
        ...
        let delRightFilter = jQuery(right_filter_element).find(".actions-container > .right-filters > .del-right-filter");
        jQuery(delRightFilter).off("click").on("click", (event) => {
            event.preventDefault();
            jQuery(right_filter_element).remove();
            jQuery(this).trigger("del-right-filter", {
                id: this.id
            });
        });
    ...
    getRightFilterElement() {
        return jQuery(`#right-filters .right-filters-container[data-id='${ this.id }']`).first();
    }
