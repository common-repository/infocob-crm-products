...
export class FilterPostMeta {
                ...
                resolve(`
                    <div class="filter post-meta" data-id="${ this.id }">
						...
						<div class="step margin ${ (this.display !== "range") ? "invisible" : "" }">
							<input type="number" data-name="step" min="0" value="${ this.step }" class="all-witdh" data-tippy-content="${ _x("Step", "Admin view catalog post, meta-box 'left-filters'", 'infocob-crm-products') }">
						</div>
    ...
    applyEvents() {
        ...
        let meta_key = jQuery(left_filter_element).find("input[data-name='meta-key']");
    ...
    getFilterElement() {
        return jQuery(`#left-filters .left-filters-container .filter.post-meta[data-id='${ this.id }']`).first();
    }
