...
import {FilterOrderBy} from "./classes/FilterOrderBy.js";

const {__, _x, _n, _nx} = wp.i18n;

export class LeftFiltersManager {
    ...
    saveEvent() {
        jQuery("#left-filters").off("update").on("update", () => {
            this.updateOrder();

            let base64Json = Utils.encodeConfig(this);
            jQuery("#left-filters > input[name='left-filters']").val(base64Json);
        });
    }

    load(configBase64) {
        jQuery("#left-filters > .content-left-filters").html("");
        jQuery("#left-filters .icp-loader").addClass("active");
    ...
    render() {
        this.toHTML().then((html) => {
            jQuery("#left-filters > .content-left-filters").html(html);

            this.applyEvents();

            jQuery("#left-filters .icp-loader").removeClass("active");
        });
    }
    ...
    applyEvents() {
        this.filters.forEach((filter, index) => {
            if(filter instanceof Filter) {
                ...
                jQuery(filter).off("del-left-filter").on("del-left-filter", (event, data) => {
        ...
        let addFilter = jQuery("#left-filters > div.actions-container > button.add-left-filter");
        jQuery(addFilter).off("click").on("click", (event) => {
            event.preventDefault();

            let filter = new Filter();
            filter.toHTML().then((filter_html) => {
                jQuery("#left-filters > .content-left-filters").append(filter_html);
                filter.applyEvents();
                this.addFilter(filter);
                ...
                jQuery("#left-filters-enable").prop("checked", true);
            });
        });
         ...
         * BEGIN Sorter events
         */
        let sortable_container = jQuery("#left-filters .content-left-filters");
    ...
    updateOrder() {
        if(this.filters !== undefined && this.filters.length > 0) {
            ...
            let filters = jQuery("#left-filters .left-filters-container");
            if(filters.length) {
