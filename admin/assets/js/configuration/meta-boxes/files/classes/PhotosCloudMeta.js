...
import {PhotosCloudMetaUtils} from "./PhotosCloudMetaUtils.js";

const {__, _x, _n, _nx} = wp.i18n;

export class PhotosCloudMeta {
	static id = 0;

	constructor() {
		this._id = PhotosCloudMeta.id;
		this._post_type = "";
		this._meta_key = "";
		this._meta_value = "";
		this._langs = [];
		...
		let langsBase64 = jQuery("#photos-cloud-meta").data("langs");
	...
	applyEvents() {
		...
		if(post_meta_element.length) {
			...
			jQuery('#photos-cloud-meta select[multiple=multiple]').multipleSelect("refresh");
		}
	}

	getPostMetaElement() {
		return jQuery(`#photos-cloud-meta .post-meta-container[data-id='${ this.id }']`).first();
	}
