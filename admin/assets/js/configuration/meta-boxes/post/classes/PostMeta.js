...
import {PostMetaUtils} from "./PostMetaUtils.js";
...
export class PostMeta {
	...
	constructor() {
		...
		let langsBase64 = jQuery("#post-meta").data("langs");
			...
			Promise.allSettled(promises).then(() => {
				resolve(`
					<div class="post-meta-container" data-id="${ this.id }">
						...
						<div class="meta">
							<div class="meta-key">
								<input data-name="meta_key" type="text" value="${ this.meta_key }" list="list-post-meta-${ this.id }-${ this.meta_key }" autocomplete="off" placeholder="${ _x("Meta key", "Admin view configuration post, meta-box 'post'", 'infocob-crm-products') }" data-tippy-content="${ _x("Add a dot before the meta key name to set properties of the post object (ex : .post_title)", "Admin view configuration post, meta-box 'post'", 'infocob-crm-products') }">
	...
	applyEvents() {
		...
		if(post_meta_element.length) {
			...
			jQuery('#post-meta select[multiple=multiple]').multipleSelect("refresh");
		}
	}

	getPostMetaElement() {
		return jQuery(`#post-meta .post-meta-container[data-id='${ this.id }']`).first();
	}
